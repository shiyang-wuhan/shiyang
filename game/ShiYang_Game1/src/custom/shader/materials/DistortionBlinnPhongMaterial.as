package custom.shader.materials {
	 ...
	 */
	public class DistortionBlinnPhongMaterial extends BaseMaterial {
		...
		public static const CURVEX:int = Shader3D.propertyNameToID("u_CurveX");
		public static const CURVEY:int = Shader3D.propertyNameToID("u_CurveY");
		...
		/** 默认材质，禁止修改*/
		public static const defaultMaterial:DistortionBlinnPhongMaterial = new DistortionBlinnPhongMaterial();
		...
		{
			_shaderValues.setNumber(CURVEX,value);
		}
		
		public function get curveX():Number
		{
			return _shaderValues.getNumber(CURVEX);
		}
		
		public function set curveY(value:Number):void
		{
			_shaderValues.setNumber(CURVEY,value);
		}
		
		public function get curveY():Number
		{
			return _shaderValues.getNumber(CURVEY);
		}
		...
		public function set enableVertexColor(value:Boolean):void {
			...
			if (value)
				_defineDatas.add(SHADERDEFINE_ENABLEVERTEXCOLOR);
			else
				_defineDatas.remove(SHADERDEFINE_ENABLEVERTEXCOLOR);
		}
		...
		public function set tilingOffset(value:Vector4):void {
			if (value) {
				if (value.x != 1 || value.y != 1 || value.z != 0 || value.w != 0)
					_defineDatas.add(SHADERDEFINE_TILINGOFFSET);
				else
					_defineDatas.remove(SHADERDEFINE_TILINGOFFSET);
			} else {
				_defineDatas.remove(SHADERDEFINE_TILINGOFFSET);
			}
		...
		public function set albedoTexture(value:BaseTexture):void {
			if (value)
				_defineDatas.add(SHADERDEFINE_DIFFUSEMAP);
			else
				_defineDatas.remove(SHADERDEFINE_DIFFUSEMAP);
		...
		public function set normalTexture(value:BaseTexture):void {
			if (value)
				_defineDatas.add(SHADERDEFINE_NORMALMAP);
			else
				_defineDatas.remove(SHADERDEFINE_NORMALMAP);
		...
		public function set specularTexture(value:BaseTexture):void {
			if (value)
				_defineDatas.add(SHADERDEFINE_SPECULARMAP);
			else
				_defineDatas.remove(SHADERDEFINE_SPECULARMAP);
		 ...
		 */
		public function DistortionBlinnPhongMaterial() {
			/*[DISABLE-ADD-VARIABLE-DEFAULT-VALUE]*/
			super();
			setShaderName("DISTORTIONBLINNPHONG");
		...
		override public function cloneTo(destObject:*):void {
			...
			var destMaterial:DistortionBlinnPhongMaterial = destObject as DistortionBlinnPhongMaterial;
